...
export class AulaEnCursoEstudianteService {
  ...
  listar(anioAulaId: string): Observable<AulaEnCursoEstudiante[]>{
    return this.httpClient.get<AulaEnCursoEstudiante[]>(`${environment.API_URL}/aula_en_curso_estudiantes/${anioAulaId}`);
  }

  registrar(aulaEnCursoEstudiante: AulaEnCursoEstudiante){
    return this.httpClient.post(`${environment.API_URL}/aula_en_curso_estudiantes`, aulaEnCursoEstudiante);
  }
  ...
  actualizarEstado(id: number, aulaEnCursoEstudiante: AulaEnCursoEstudiante): Observable<any>{
    return this.httpClient.patch(`${environment.API_URL}/aulas_en_curso/${id}`, aulaEnCursoEstudiante)
  }
}
